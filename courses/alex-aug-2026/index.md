---
layout: home
title: Alex Aug 2026
---

# Welcome to CS50 at ITI
## Alexandria - August 2026 Cohort

This is the homepage for the custom **Alexandria August 2026** cohort. 

This program is a customized version of the curriculum tailored for your training track. It focuses on the fundamental concepts of computer science (Scratch and C++) and transitions into full Web Development (HTML, CSS, and JavaScript).

### Course Navigation
Please use the sidebar menu to navigate the course contents:
- **Lecture PDFs:** Download the presentation slides for your lectures.
- **Lecture Tasks:** Access the hands-on code files and descriptions for class lectures.
- **Problem Sets:** Complete and submit your coding assignments here.
